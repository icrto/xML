...
def unsupervised_explanation_loss(pred, beta):
    """ Calculates the unsupervised explanation loss for a single prediction
...
def hybrid_explanation_loss(pred, target, beta, gamma):
    """ Calculates the hybrid explanation loss for a single prediction
...
def batch_unsupervised_explanation_loss(preds, beta, reduction='mean'):
    """ Calculates the unsupervised explanation loss for one mini-batch
...
def batch_hybrid_explanation_loss(preds, targets, beta, gamma, reduction='mean'):
    """ Calculates the hybrid explanation loss for one mini-batch
    ...
    for idx, pred in enumerate(preds):
        loss += hybrid_explanation_loss(pred.squeeze(),
                                        targets[idx][0], beta, gamma)
