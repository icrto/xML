...
class Dataset(torch.utils.data.Dataset):
    """ Class Dataset
    """
    def __init__(self, df, preprocess=utils.norm, masks=False, img_size=(224, 224), aug_prob=0):
...
def load_synthetic_dataset(folder, dataset='simplified_no_colour', masks=False, class_weights=None):
    ...
    df['imageID'] = [os.path.join(dtset_folder, datum)
                     for datum in df.imageID.values]
...
def load_NIH_NCI(folder, masks=False, class_weights=None):
    ...
    for subpath in ['ALTS', 'Biopsy', 'CVT', 'NHS']:
        df = pd.read_excel(os.path.join(
            img_path, 'data', subpath, 'covariate_data.xls'), header=4)
        ...
        for i in np.arange(df.shape[0]):
            row = df.iloc[i]
            df.loc[i, 'path'] = os.path.join(
                img_path, 'data2', subpath, row.GG_IMAGE_ID)
    ...
    df_['imageID'] = [p[:-4] + '_resized.jpg' for p in dft.path]
    ...
    train_session_df = train_df[['sessionID', 'label']].groupby(
        'sessionID').agg('max')
