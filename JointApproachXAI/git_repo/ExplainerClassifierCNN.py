...
class ExplainerClassifierCNN(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), num_classes=2, clf='resnet50', init_bias=3.0, pretrained=False):
        ...
        if('resnet152' in self.clf.lower()):
            self.decmaker = resnet152(
                pretrained=self.pretrained, progress=True, img_size=img_size, num_classes=num_classes)
    ...
    def train(self, dataloader, optimiser, device, args, phase, weights, alpha, disable=False):

        self.decmaker.train()
        self.explainer.train()

        dec_criterion = torch.nn.CrossEntropyLoss(
        ...
        for batch_imgs, batch_labels, _, batch_masks in tqdm(dataloader, disable=disable):
            ...
            if(len(batch_masks) > 0):  # hybrid loss
                batch_imgs, batch_labels, batch_masks = batch_imgs.to(
                    device), batch_labels.to(device), batch_masks.to(device)
            else:
            ...
            if(args.loss == 'unsupervised'):
                ...
                exp_loss = Losses.batch_hybrid_explanation_loss(
                    batch_expls, batch_masks, float(args.beta), float(args.gamma), reduction='mean')
            ...
            for batch_imgs, batch_labels, _, batch_masks in tqdm(dataloader, disable=disable):
                ...
                if(args.loss == 'unsupervised'):
                    ...
                    batch_exp_loss = Losses.batch_hybrid_explanation_loss(
                        batch_expls, batch_masks, float(args.beta), float(args.gamma), reduction='sum')

                batch_loss = float(alpha) * batch_dec_loss + \
                    (1-float(alpha)) * batch_exp_loss
    ...
    def save_explanations(self, dataloader, phase, device, path, disable=False, test=False, classes=None, cmap=None):
        ...
        with torch.no_grad():
            ...
            for batch_imgs, batch_labels, batch_names, _ in tqdm(dataloader, disable=disable):
                ...
                for idx, img in enumerate(batch_imgs):  # transverses the batch
                    ...
                    expl = np.swapaxes(
                        batch_expls[idx].squeeze().cpu().numpy(), 0, 1)

                    string = 'Label: ' + classes[batch_labels[idx]] + '\n'
                    for i, c in enumerate(classes):
                        string += c + ' ' + \
                            str(round(batch_probs[idx][i].squeeze(
                            ).cpu().numpy().item(), 6)) + '\n'
                    ...
                    plt.text(0.91, 0.5, string, fontsize=12,
                             transform=plt.gcf().transFigure)
                    if(test):
                        plt.savefig(os.path.join(path, '{}_phase{}_ex_img_test_{}_{}_{}'.format(timestamp, str(
                            phase), mode, cmap, batch_names[idx].split('/')[-1])), bbox_inches='tight', transparent=True, pad_inches=0.1)
                    else:
                        plt.savefig(os.path.join(path, '{}_phase{}_ex_img_{}'.format(timestamp, str(
                            phase), batch_names[idx].split('/')[-1])), bbox_inches='tight', transparent=True, pad_inches=0.1)
                    plt.close()
