...
parser.add_argument('--folder', type=str, default='/media/TOSHIBA6T/ICRTO/JointApproach/git_test',
...
parser.add_argument('-lr_clf', '--learning_rate_classifier', type=str, default='1e-3,0,1e-4',
                    help='Learning rate for each training phase of the classifier, for example: 1e-3,0,1e-4.')
parser.add_argument('-lr_expl', '--learning_rate_explainer', type=str, default='0,1e-4,1e-4',
...
parser.add_argument('--early_delta', type=int, default=1e-4,
...
summary(model.decmaker, [(3, 224, 224), (1, 224, 224)], filename=os.path.join(
    path, timestamp + '_model_dec_info.txt'))
summary(model.explainer, (3, 224, 224), filename=os.path.join(
    path, timestamp + '_model_exp_info.txt'))
...
tr_df, val_df, _, weights, classes = load_data(folder=args.dataset_path, dataset=args.dataset,
                                               masks=masks, class_weights=class_weights)
weights = torch.FloatTensor(weights).to(device)

train_dataset = Dataset(tr_df, masks=masks,
                        img_size=args.img_size, aug_prob=args.aug_prob)
...
for phase in range(3):
    ...
    early_stopping = EarlyStopping(
        patience=early_patience[phase], delta=args.early_delta, verbose=True, folder=path, timestamp=timestamp)
    ...
    with open(history_file, 'a+') as f:
        ...
        csv_writer.writerow(['epoch', 'train_global_loss', 'train_decmaker_loss', 'train_explainer_loss',
                             'train_decmaker_acc', 'val_global_loss', 'val_decmaker_loss', 'val_explainer_loss', 'val_decmaker_acc'])

    # select optimizer
    if(args.opt == 'adadelta'):
        opt = optim.Adadelta([{'params': model.decmaker.parameters(),
                               'lr': lr_clf[phase]},
                              {'params': model.explainer.parameters(),
                               'lr': lr_expl[phase]}
                              ], weight_decay=args.decay)
    else:
        opt = optim.SGD([{'params': model.decmaker.parameters(),
                          'lr': lr_clf[phase]},
                         {'params': model.explainer.parameters(),
                          'lr': lr_expl[phase]}
    ...
    if('resnet' in args.classifier.lower()):
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min',
                                                         factor=args.factor, patience=args.patience, verbose=True, min_lr=args.min_learning_rate)
    else:
        scheduler = None
    ...
    for epoch in range(nr_epochs[phase]):  # training loop for each phase
        ...
        print('Train')
        model.train(train_loader, opt, device, args,
                    phase, weights, alpha[phase])

        print('Val')
        # pass entire training set through validation function to obtain metrics at the end of the training epoch
        train_global_loss, train_explainer_loss, train_decmaker_loss, train_decmaker_acc = model.validation(
            train_loader, device, args, alpha[phase])
        print('Train Loss %f \tTrain Exp Loss %f \tTrain Dec Loss %f \tTrain Acc %f' % (
            train_global_loss, train_explainer_loss, train_decmaker_loss, train_decmaker_acc))

        val_global_loss, val_explainer_loss, val_decmaker_loss, val_decmaker_acc = model.validation(
            val_loader, device, args, alpha[phase])
        print('Val Loss %f \tVal Exp Loss %f \tVal Dec Loss %f \tVal Acc %f' % (
            val_global_loss, val_explainer_loss, val_decmaker_loss, val_decmaker_acc))

        print()

        # save model and epoch metrics
        model.checkpoint(checkpoint_filename, epoch,
                         val_global_loss, val_decmaker_acc, opt)
    ...
    print()
    utils.plot_metric_train_val(epoch + 1, history_file, 'decmaker_loss', path, os.path.join(
        path, timestamp + '_phase' + str(phase) + '_decmaker_loss.png'), 'Classifier Loss')
    utils.plot_metric_train_val(epoch + 1, history_file, 'explainer_loss', path, os.path.join(
        path, timestamp + '_phase' + str(phase) + '_explainer_loss.png'), 'Explainer Loss')
    utils.plot_metric_train_val(epoch + 1, history_file, 'global_loss', path, os.path.join(
        path, timestamp + '_phase' + str(phase) + '_global_loss.png'), 'Global Loss')
    utils.plot_metric_train_val(epoch + 1, history_file, 'decmaker_acc', path, os.path.join(
        path, timestamp + '_phase' + str(phase) + '_decmaker_acc.png'), 'Accuracy')
    model.save_explanations(val_loader, phase, device,
                            path, classes=classes, cmap='viridis')
