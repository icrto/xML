...
class VGGClf(nn.Module):
    ...
    def __init__(self, in_channels=3, dec_conv_filter_size=(3, 3), dec_pool_size=2, img_size=(224, 224), dropout=0.3, num_classes=2):
        ...
        self.exp_pool = nn.AvgPool2d(self.dec_pool_size) # explanations are downsampled using average pooling --> see paper for details
    ...
    def forward(self, x, expl):
        last_expl = expl
        x = self.conv_mod0(x)
        x = torch.mul(x, torch.cat([last_expl] * 32, dim=1)) # connection between explainer and classifier
        x = self.clf_pool(x)
        last_expl = self.exp_pool(last_expl)

        x = self.conv_mod1(x)
        x = torch.mul(x, torch.cat([last_expl] * 64, dim=1)) # connection between explainer and classifier
        x = self.clf_pool(x)
        last_expl = self.exp_pool(last_expl)

        x = self.conv_mod2(x)
        x = torch.mul(x, torch.cat([last_expl] * 128, dim=1)) # connection between explainer and classifier
        x = self.clf_pool(x)
        last_expl = self.exp_pool(last_expl)

        x = self.conv_mod3(x)
        x = torch.mul(x, torch.cat([last_expl] * 256, dim=1)) # connection between explainer and classifier
        x = self.clf_pool(x)
