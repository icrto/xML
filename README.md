...
<ul>
  ...
  <li><p align="justify"><a href="https://github.com/icrto/xML/blob/master/PyTorch/test.py">test.py</a> - allows the user to test a trained model given by a previously saved PyTorch checkpoint (.pt file). The script starts by validating the parameters, loading the model and creating the <code>test dataloader</code>. Having done this, we simply need to call the function <code>test</code> and save our results to a .txt file, as well as save the produced explanations and the roc and precision-recall curves.</p></li>
